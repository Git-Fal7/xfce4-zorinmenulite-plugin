	...
	Page(window)
{
	...
	m_search_action = new SearchAction(_("Open URI"), "^(file|http|https):\\/\\/(.*)$", "exo-open \\0");
...
void SearchPage::set_filter(const gchar* filter)
{
	...
	search_action_matches.reserve(1);
	Match match(m_search_action);
