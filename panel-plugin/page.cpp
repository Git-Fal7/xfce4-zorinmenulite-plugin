	...
	m_selected_path(NULL)
{
	...
	m_widget = GTK_WIDGET(gtk_box_new(GTK_ORIENTATION_VERTICAL, 6));
	m_scroll = gtk_scrolled_window_new(NULL, NULL);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(m_scroll), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(m_scroll), GTK_SHADOW_ETCHED_IN);
	gtk_container_add(GTK_CONTAINER(m_scroll), m_view->get_widget());
	gtk_box_pack_start(GTK_BOX(m_widget), GTK_WIDGET(m_scroll), true, true, 0);
	g_object_ref_sink(m_widget);

	// Create Back Button
	m_back_button = GTK_BUTTON(gtk_button_new());
	gtk_button_set_relief(GTK_BUTTON(m_back_button), GTK_RELIEF_NONE);

	m_button_box = GTK_BOX(gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 8));
	gtk_container_add(GTK_CONTAINER(m_back_button), GTK_WIDGET(m_button_box));

	m_back_icon = gtk_image_new_from_icon_name("go-previous-symbolic", GTK_ICON_SIZE_BUTTON);
	gtk_box_pack_start(m_button_box, m_back_icon, false, false, 0);

	m_back_label = gtk_label_new(_("Back"));
	gtk_box_pack_start(m_button_box, m_back_label, false, true, 0);
	gtk_widget_show(m_back_label);

	gtk_box_pack_start(GTK_BOX(m_widget), GTK_WIDGET(m_back_button), false, false, 0);
	g_signal_connect_slot<GtkButton*>(m_back_button, "clicked", &Page::back_button_activated, this);
}
...
Page::~Page()
{
	...
	delete m_view;
	gtk_widget_destroy(GTK_WIDGET(m_back_button));
...
void Page::back_button_activated()
{
	m_window->back_button_activated();
}
